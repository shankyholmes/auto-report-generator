...
#   f2 = c(f2,substr(file_names[i],1,nchar(file_names[i])-4))
# }
# fn = paste(f2,collapse = "\n")
# 
# k2 = textGrob(fn,gp = gpar(fontsize = 14))
...
#tg = ggtexttable(ctdata, rows = NULL, theme = ttheme("mRed"))
...
RDPlot = ggplot(dfm,aes(x=dfm$Strt.Count,y=value,fill = variable))+
  ...
  theme(axis.title.x = element_text(family = "serif"),axis.title.y = element_text(family = "serif"),
        title = element_text(family = "serif"),plot.title = element_text(face="bold"),plot.subtitle = element_text(face="bold"))
...
T5Plot = ggplot(k2,aes(x=k2$Strt.Count,y=k2$T5_Avg))+
  ...
  theme(axis.title.x = element_text(family = "serif"),axis.title.y = element_text(family = "serif"),
        title = element_text(family = "serif"),legend.position = "none",plot.title = element_text(face="bold"),plot.subtitle = element_text(face="bold"))
